...
# form the prediction
#
assign("pred.frame", pred.frame, pos=1, immediate =T)
...
{ vert.limits <- z.range}
  ...
  plot(pred.frame[pred.frame[,1]==factor.list[1],2],
      ...
      xlab = y.label,
      ylab = z.label, ...)
  for (i in 2:length(factor.list)) { #factor.level in factor.list) {
    factor.level <- factor.list[i]
    lines(pred.frame[pred.frame[,1]==factor.level,2],
      prediction[pred.frame[,1]==factor.level],
      lty = i)
    }
...
if(is.null(leg.coords)){
  ...
  legend(x = leg.coords[1], y = leg.coords[2], factor.list, lty = c(1:length(factor.list)),
    bty = "n")
}
  }
}
