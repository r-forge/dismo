...
Above we used the function gbm.step, this function is an alternative to the cross-validation one that Ridgeway provides.  
...
<<dismo-6>>=
gbm.plot(angaus.tc5.lr005, n.plots=11, write.title = F)
@
...
<<dismo-10>>=
data(Anguilla_test)
preds <- predict.gbm(angaus.tc5.lr005, Anguilla_test, n.trees=angaus.tc5.lr005$gbm.call$best.trees, type="response")
...
<<dismo-11>>=
angaus.5000 <- gbm.fixed(data=Anguilla_train, gbm.x=3:13, gbm.y=2, learning.rate=0.005, tree.complexity=5, n.trees=5000)
...
p = predict(Anguilla_grids, angaus.tc5.lr005, n.trees=angaus.tc5.lr005$gbm.call$best.trees, type="response", const=add)
