...
1) use the gbif function to download records for the hippopotamus. Use option "geo=FALSE" to also get records with no (numerical) georeference.

2) summarize the data: how many records, how many have coordinates, how many without coordinates have a textual georeference (locality description)?

4) use the geocode function to georeference records without coordinates (how many new mappalble records did you get?)

5) make a simple map of all the records, using a color and symbol to distinguish between the coordinates from gbif and the ones from google

6) use the 'rasterize' function to create a raster of the number of observations and map that

7) do you think the observations are a reasonable representation of the distribution (and ecological niche) of the hippo?
