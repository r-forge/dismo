...
At this point, it could be useful to save the cleaned data set. For example with the function \texttt{write.table} or \texttt{write.csv} so that we can use them later. The same file is available through dismo and can be retrieved like this:
...
Seperate model predictions:

<<sdm82, fig=TRUE, width=9, height=6>>=
models <- stack(pb, pd, pm, pg, px, pr, ps)
names(models) <- c("bioclim", "domain", "mahal", "glm", "maxent", "rf", "svm")
plot(models)
@
