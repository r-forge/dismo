...
Background data for training and a testing set. The first layer in the RasterStack is used as a 'mask'. That ensures that random points only occur within the spatial extent of the rasters, and within cells that are not NA, and that there is only a single absence point per cell.

<<sdm32, echo=TRUE>>=
backg <- randomPoints(pred_nf, 1000)
group <- kfold(bg, 5)
...
And evaluate it in a similar way:
<<sdm34, echo=FALSE>>=
e <- evaluate(pres_test, backg_test, bc, pred_nf)
e
@

And make a prediction to a RasterLayer:
<<sdm35, fig=TRUE>>=
p = predict(bc, pred_nf, progress='tcltk')
p
plot(p)
plot(wrld_simpl, add=TRUE, border='dark grey')
points(pres_train, pch='+')
points(pres_test, col='blue', pch=20 )
@
