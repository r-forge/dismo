...
<<sdm36, fig=TRUE, width=9, height=6>>=
...
plot(wrld_simpl, add=TRUE, border='dark grey')
threshold <- e1@t[which.max(e@TPR + e@TNR)]
...
Because maxent is implemented in dismo you can fit it like the profile methods (e.g. Bioclim). That is, you can provide presence points and a RasterStack. 
However, you can also fit it like the other methods, such as glm. Note, however, that in that case you cannot use the formula notation.
