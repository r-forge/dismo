...
Here is how you could do some data preparation and cleaning before using georef:

<<sdm10>>=
loc <- apply(georef[, c('locality', 'adm1', 'adm2', 'country')], 1, function(x) paste0(x, collapse=', '))
loc <- gsub("NA,", "", loc)
loc <- gsub("NA", "", loc)
loc <- gsub("PER", "Peru", loc)
loc <- gsub("BOL", "Bolivia", loc)
loc <- gsub("ARG", "Argentina", loc)
loc <- trim(loc)
uloc <- unique(loc)
#georef(uloc[1:5])
@ 
...
\noindent 2) summarize the data: how many records, how many have coordinates, how many without coordinates have a textual georeference (locality description)?
...
\noindent 5) use the 'rasterize' function to create a raster of the number of observations and make a map. Use "wrld\_simpl" from the maptools package for country boundaries.

\noindent 6) do you think the observations are a reasonable representation of the distribution (and ecological niche) of the hippo?
